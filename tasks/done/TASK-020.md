...
review_conclusion: |
  ...
  4. EnsureSchemaAsync 鍖呭惈 schema 鍚屾 + 瀵嗙爜淇 + 榛樿鐢ㄦ埛
...
# TASK-020: 鐧诲綍瀵嗙爜鏍￠獙閫昏緫淇 鈥?MD5 鏍煎紡妫€娴?+ 鍏煎鏄庢枃

## 鐜拌薄

杈撳叆瀵嗙爜 `xwxy51020` 鐧诲綍鎻愮ず"璐﹀彿鎴栧瘑鐮侀敊璇?銆?
...
1. **鏁版嵁搴撳彲鑳戒粛瀛樻槑鏂?*锛歚app_user` 琛ㄤ腑 xinke.wang 瀵嗙爜瀛楁鑻ユ湭鎵ц UPDATE 浠嶄负鏄庢枃 `"xwxy51020"`锛屼唬鐮佸皢鍏?MD5 鍝堝笇鍚庝笌鏄庢枃姣斿锛屽繀鐒跺け璐?
2. **MD5 杈撳叆琚簩娆″搱甯?*锛氳嫢鐢ㄦ埛鐩存帴杈撳叆 32 浣?MD5 鍊肩櫥褰曪紝浼氳鍐嶆 ToMd5锛屽繀鐒跺け璐?

## 淇敼浠诲姟

### LoginAsync 澧炲姞 MD5 鏍煎紡鍒ゆ柇

淇敼浣嶇疆: `src/ArasToolkit.Services/Services/AppUserService.cs` 绗?24-34 琛?
...
public async Task<AppUser?> LoginAsync(string username, string password)
{
    ...
    // 鍒ゆ柇杈撳叆鏄惁宸叉槸 32 浣嶅皬鍐欏崄鍏繘鍒?MD5 鏍煎紡
...
/// <summary>
/// 鍒ゆ柇瀛楃涓叉槸鍚﹀凡鏄?32 浣嶅皬鍐欏崄鍏繘鍒?MD5 鏍煎紡
/// </summary>
...
**閫昏緫璇存槑**锛?
- 鐢ㄦ埛杈撳叆 `xwxy51020`锛堟槑鏂囷級鈫?`IsMd5Format` 杩斿洖 false 鈫?ToMd5 寰楀埌 `915d92dbc92c8b655764d3df7e22161b` 鈫?涓?DB 姣斿 鈫?鎴愬姛
- 鐢ㄦ埛杈撳叆 `915d92dbc92c8b655764d3df7e22161b`锛圡D5锛夆啋 `IsMd5Format` 杩斿洖 true 鈫?鐩存帴姣斿 鈫?鎴愬姛
- 鏁版嵁搴撴棤璁哄瓨 MD5 杩樻槸鏄庢枃锛堣繃娓℃湡锛夛紝閮借兘鍖归厤
...
```bash
git add -A
git commit -m "淇: TASK-020 鐧诲綍瀵嗙爜鏍￠獙澧炲姞MD5鏍煎紡妫€娴嬶紝鍏煎鏄庢枃杈撳叆"
```
