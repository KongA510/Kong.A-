...
# TASK-018: 閿欒鏃ュ織鏃犱换浣曟暟鎹?鈥?琛ㄦ湭鍒涘缓 + OnModelCreating 缂烘槧灏?
## 鐜拌薄

閿欒鏃ュ織椤甸潰锛圗rrorLogView锛夊缁堜负绌猴紝鏌ヨ涓嶅埌浠讳綍閿欒璁板綍銆?
## 鏍瑰洜鍒嗘瀽
...
ArasToolkitDbContext.EnsureSchemaAsync() 鐨勮皟鐢ㄧ偣:
  - TodoService.cs:201 鈫?鐢ㄦ埛杩?Todo 妯″潡鏃舵墠璋冪敤
  - SettingsViewModel.cs:85 鈫?淇濆瓨杩炴帴璁剧疆鏃舵墠璋冪敤

鈫?鐢ㄦ埛濡傛灉娌¤繘鍏?Todo銆佹病淇濆瓨杩炴帴璁剧疆锛宔rror_log 琛ㄤ粠鏈垱寤?鈫?ErrorLogService.LogErrorAsync() 鎵ц INSERT 澶辫触
鈫?catch 鍧楅潤榛樹涪寮?鈫?閿欒鏃ュ織姘歌繙涓虹┖
```
...
褰撳墠浠呬緷璧栧疄浣撲笂鐨?`[Column("user_name")]` 娉ㄨВ鍏滃簳锛屼絾鍏朵粬瀹炰綋閮芥槸 Fluent API + 娉ㄨВ鍙岄噸瑕嗙洊锛屽簲琛ラ綈銆?
## 淇敼浠诲姟

### 1. AppLoginViewModel 涓紝搴旂敤鐧诲綍鎴愬姛鍚庤皟鐢?DbContext.EnsureSchemaAsync()

淇敼浣嶇疆: `src/ArasToolkit.App/ViewModels/AppLoginViewModel.cs`
...
淇敼浣嶇疆: `src/ArasToolkit.Services/Data/ArasToolkitDbContext.cs`
...
- `dotnet build ArasToolkit.slnx` 闆堕敊璇?- 鍚姩搴旂敤 鈫?鐧诲綍 鈫?瑙﹀彂涓€涓敊璇紙濡傞敊璇瘑鐮佺櫥褰旳ras锛夆啋 杩涘叆閿欒鏃ュ織椤甸潰 鈫?鑳界湅鍒拌褰?- 鐩存帴鏌ユ暟鎹簱 `SELECT * FROM error_log` 纭琛ㄥ凡鍒涘缓涓旀湁鏁版嵁
...
绗?椤逛粛鏈慨鏀癸細AppLoginViewModel 鏈敞鍏?IDbContextFactory锛孖nitializeAsync 涓湭璋冪敤 EnsureSchemaAsync銆?
