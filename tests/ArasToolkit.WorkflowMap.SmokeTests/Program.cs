...
internal static class Program
{
    private static async Task Main(string[] args)
    {
        ...
        try
        {
            ...
            Assert(returnPath.Segments == "390,75|220,75", "模板退回线应保留两个原生 segments 转折点。");
            ...
            Assert(generatedBends.All(point => point.Y < 170), "自动退回折点应位于主线上方。");
            ...
            Assert(CountItems(aml, "Activity Template Assignment") == 3, "AML 应包含 3 个人工节点角色关系。");
            ...
            Assert(innovator.QueryCalls == 1 && innovator.ImportCalls == 0, "生成 AML 阶段只应查询一次同名模板。");
            ...
            Assert(innovator.QueryCalls == 2 && innovator.ImportCalls == 2,
                "新增与覆盖每次都应只查询一次、导入一次。");
            Assert(operationLog.Entries.Count == 2, "新增和覆盖应各记录一条 Import 操作日志。");
            Assert(errorLog.Entries.Count == 0, "成功回归过程不应产生错误日志。");

            Console.WriteLine("工作流程设定回归测试通过：6 节点 / 6 路径 / segments 转折点 / 新增与覆盖 AML 单次执行。");
        }
        finally
        {
    ...
    {
        const string description = "新增工作流程设定：Excel可编辑预览、segments转折点、完整AML新增与覆盖汇入";
...
public sealed class FakeArasResult
{
    ...
    public FakeArasResult(params FakeMapItem[] items) => _items = [.. items];
