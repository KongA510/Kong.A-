...
1. **本地提交 → 本地备份 → 等待用户确认 → 才能推送远程：**
   - 每次修改完成后：`git add -A && git commit`
   - 本地备份到 develop：`git checkout develop && git merge master && git checkout master`
   - **⚠️ 禁止自动推送远程！** 必须等用户明确说"推送"后才执行 `git push origin master --force`
   - 目的：避免未验证的代码污染云端主线

2. **推送远程前的检查清单：**
   - ✅ `git status` 检查是否有未提交的改动（用户可能手动编辑了文件）
   - ✅ 如有未提交改动，先 `git add -A && git commit` 保存用户的手动修改
   - ✅ `dotnet build` 编译通过（0 错误）
   - ✅ 用户已确认"可以推送"或"推送到远程"
   - ✅ develop 已备份当前 master
   - 然后才执行：`git push origin develop` + `git push origin master --force`
   - **⚠️ 绝对不能在推送前执行 `git reset --hard`，否则用户手动改动会丢失！**

3. **master 使用 force push：**
   - 本地 master 是唯一代码真相来源
