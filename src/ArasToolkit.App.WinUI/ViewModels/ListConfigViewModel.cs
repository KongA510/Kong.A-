...
/// <summary>
/// List配置页面 ViewModel
/// 负责: Excel模板下载（3Sheet）、文件选择、批量导入Aras、导入历史分页查询
/// 功能与对象类配置完全一致，仅 AML 部分由调用方自行维护
/// </summary>
public class ListConfigViewModel : ObservableObject
{
    private readonly IListImportService _importService;
    ...
    // ===== 导入结果 =====
    private ListImportResult? _lastResult;
    ...
    public ListConfigViewModel(
        IListImportService importService,
        ...
        IFileDialogService fileDialogService)
    {
        ...
        HistoryRecords = new ObservableCollection<ListImportLog>();
    ...
    public ListImportResult? LastResult
    {
        get => _lastResult;
        set
        {
    ...
    public ObservableCollection<ListImportLog> HistoryRecords { get; }
    ...
    private async Task DownloadTemplateAsync()
    {
        try
        {
            var filePath = await _fileDialogService.PickSaveFileAsync("List配置模板.xlsx", ".xlsx");
        ...
        catch (Exception ex)
        {
            ...
            _ = _errorLogService.LogErrorAsync("List配置-下载模板", ex.Message,
    ...
    {
        var filePath = await _fileDialogService.PickOpenFileAsync("选择要导入的 List 配置 Excel 文件", ".xlsx", ".xls");
        if (filePath != null)
    ...
    private async Task ExecuteImportAsync()
    {
        ...
        try
        {
            ...
            if (LastResult.IsSuccess)
            {
                StatusMessage = LastResult.HasFailures
                    ? $"导入完成（部分失败）: List主档{LastResult.Sheet1Count}/{LastResult.Sheet1Total} | 菜单{LastResult.Sheet2Count}/{LastResult.Sheet2Total} | 过滤{LastResult.Sheet3Count}/{LastResult.Sheet3Total}"
                    : $"导入成功: List主档{LastResult.Sheet1Count}条 / 菜单{LastResult.Sheet2Count}条 / 过滤{LastResult.Sheet3Count}条";
            }
            else
            {
        ...
        catch (OperationCanceledException)
        {
            ...
            await _errorLogService.LogErrorAsync("List配置-导入取消", "用户取消导入",
    ...
    private void CancelImport()
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[ListConfig] 取消导入异常: {ex.Message}");
        }
    }
    ...
    private async Task LoadHistoryAsync()
    {
        ...
        catch (Exception ex)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"[ListConfig] 加载历史失败: {ex.Message}");
        }
    }
}
