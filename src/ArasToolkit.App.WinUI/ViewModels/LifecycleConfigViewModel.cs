...
public class LifecycleConfigViewModel : ObservableObject
{
    ...
    private async Task DownloadTemplateAsync()
    {
        try
        {
            var filePath = await _fileDialogService.PickSaveFileAsync("生命周期配置模板.xlsx", ".xlsx");
    ...
    {
        var filePath = await _fileDialogService.PickOpenFileAsync("选择要导入的生命周期配置 Excel 文件", ".xlsx", ".xls");
        if (filePath != null)
