...
public class TodoViewModel : ObservableObject
{
    ...
    public TodoViewModel(ITodoService todoService, IErrorLogService errorLogService, IDialogService dialogService, IFileDialogService fileDialogService)
    {
    ...
    private async Task DeleteItemAsync(string id)
    {
        ...
        var confirmed = await _dialogService.ConfirmAsync("确认删除", "确定要删除该待办项吗？", "删除", "取消");
        if (!confirmed) return;
    ...
    {
        var filePath = await _fileDialogService.PickOpenFileAsync("选择个人任务记录Excel文件", ".xlsx", ".xls");
    ...
    {
        var filePath = await _fileDialogService.PickSaveFileAsync("个人任务记录导入模板.xlsx", ".xlsx");
        if (filePath == null) return;
