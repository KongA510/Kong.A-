...
/// <summary>
/// 错误日志ViewModel - 分页 + 筛选（WinUI 3）。
/// 说明：WinUI DatePicker.SelectedDate 为 DateTimeOffset?，故筛选日期属性改用 DateTimeOffset?，
/// 调用服务时转换为 DateTime?，业务行为与 WPF 版完全一致。
/// </summary>
public class ErrorLogViewModel : ObservableObject
{
    ...
    private DateTimeOffset? _filterFromDate, _filterToDate;
    ...
    public DateTimeOffset? FilterFromDate { get => _filterFromDate; set => SetProperty(ref _filterFromDate, value); }
    public DateTimeOffset? FilterToDate { get => _filterToDate; set => SetProperty(ref _filterToDate, value); }
    ...
    private async Task LoadDataAsync()
    {
        ...
        try
        {
            var (items, total) = await _errorLogService.GetPagedEntriesAsync(
                CurrentPage, _pageSize, FilterLevel, FilterFromDate?.DateTime, FilterToDate?.DateTime);
